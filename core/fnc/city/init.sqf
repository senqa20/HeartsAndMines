...
for "_i" from 0 to (count _locations - 1) do {
	...
	if (_type in _cities) then {
		...
		if (_position distance getMarkerPos "marker_0" < 700) exitWith {};
		if (_position distance getMarkerPos "marker_1" < 700) exitWith {};
		if (_position distance getMarkerPos "marker_2" < 700) exitWith {};
